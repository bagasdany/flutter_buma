...
class AllNewsUser extends StatefulWidget {
  dynamic id,name;
  AllNewsUser({Key? key, this.id,this.name}) : super(key: key);
  final GlobalKey<RefreshIndicatorState> _refreshIndicatorKey =
      GlobalKey<RefreshIndicatorState>();

  @override
  _AllNewsUserState createState() => _AllNewsUserState();
}

class _AllNewsUserState extends State<AllNewsUser> {
  ...
  Future<void> saveMap(Map<String, dynamic> data) async {
    Map dataliked =  await readMap();
    dataliked.addAll(data);
    final prefs = await SharedPreferences.getInstance();
    final jsonString = jsonEncode(dataliked);
  ...
  void _firstLoad() async {
    ...
    try {
      ...
      Response response = await _dio.get("/post", queryParameters: params);
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        ...
        Response res = await _dio.get("/post", queryParameters: params);
  ...
  Widget searchPost(){
    return Container(
      padding: const EdgeInsets.fromLTRB(Constants.spacing4, 0, Constants.spacing4, 0),
      child: Focus(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                InkWell(
                  onTap: () {
                    item['liked'] = item['liked'] ==null ? true : false;
                    setState(() {
                      if(item['liked'] == true){
                        item['liked'] = false;
